...
MB.daily <- rio::import(here("data", "daily_viral_load", 
                             "MB_daily_viral_load.csv"), 
                        header = TRUE)
QC.daily <- rio::import(here("data", "daily_viral_load",
...
MB.weekly <- rio::import("data/weekly_mavg/MB_7DayAvg.csv",
                         header = TRUE)
QC.weekly <- rio::import("data/weekly_mavg/QC_7DayAvg.csv",
                         header = TRUE)
...
ls.daily.canada <- list.files(path = "data/daily_viral_load/", full.names = TRUE)
...
ls.weekly.canada <- list.files(path = "data/weekly_mavg/", full.names = TRUE)
# note: full.names argument is required to list the path to the files
...
pruid <- rio::import(here("data", "demographics", "pruid.csv"), 
                     header = TRUE)
str(pruid)



region.pop <- rio::import(here("data", "demographics", 
...
write_csv(covid.ww.final, here("data", "covid_ww.csv"))
# Hurray! 
